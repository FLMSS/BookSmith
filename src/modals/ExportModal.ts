import { Notice, App, Modal, TFile } from "obsidian";
...
interface ExportCleanupOptions {
...
export class ExportModal extends Modal {
    private static readonly MODAL_SHELL_CLASS = 'book-smith-export-modal-shell';
    private static readonly MODAL_INIT_CLASS = 'book-smith-export-modal-initializing';
    // UI 元素引用
    ...
    private removeLinks: boolean = true;
    private removeFrontmatter: boolean = true;
    ...
    constructor(
        ...
        super(app);
        this.modalEl.addClass(ExportModal.MODAL_SHELL_CLASS);
        // 初始化封面设置默认值
    ...
    onOpen() {
        ...
        this.modalEl.addClass(ExportModal.MODAL_INIT_CLASS);
        contentEl.empty();
        ...
        window.requestAnimationFrame(() => {
            this.modalEl.removeClass(ExportModal.MODAL_INIT_CLASS);
        });
    }
    ...
    onClose() {
        ...
        this.cleanupWebview();
        this.modalEl.removeClass(ExportModal.MODAL_INIT_CLASS);
        this.modalEl.removeClass(ExportModal.MODAL_SHELL_CLASS);
    ...
    private createPreviewArea() {
        ...
        headerContent.createEl('h3', { text: 'PDF Export Preview', cls: 'preview-title' });
        // 创建一个包含封面和内容的滚动容器
        ...
        switch (state) {
            ...
            case 'loading':
                ...
                loading.innerHTML = `
                    ...
                    <div class="preview-loading-text">Rendering preview...</div>
            ...
            case 'error':
                ...
                error.innerHTML = `
                    ...
                    <div class="preview-error-text">${errorMessage || 'Rendering failed, please retry'}</div>
                    <button class="preview-retry-btn">Re-render</button>
                `;
    ...
    private async startRenderPreview() {
        ...
        try {
            ...
            if (this.isRendering) {
                ...
                errorEl.innerHTML = `
                    <div class="preview-error-icon">❌</div>
                    <div class="preview-error-text">${error.message || 'Rendering failed, please retry'}</div>
                    <button class="preview-retry-btn">Re-render</button>
                `;
                ...
                new Notice('Rendering failed, please check console error message');
            }
        } finally {
            if (!this.abortController?.signal.aborted) {
    ...
    private updateExportButtonState() {
        if (this.exportBtn) {
            ...
            this.exportBtn.textContent = this.isRendering ? 'Rendering...' : 'Export';
        }
    }
    ...
    private createBookInfo(container: HTMLElement) {
        ...
        if (this.selectedBook.basic.author && this.selectedBook.basic.author.length > 0) {
            bookDetails.createEl('p', {
                text: `Author: ${this.selectedBook.basic.author.join(', ')}`,
    ...
    private createFormatSelection(container: HTMLElement) {
        ...
        const formats = [
            { key: 'markdown', label: 'Markdown (.md)', icon: '📝', desc: i18n.t('MARKDOWN_FORMAT_DESC') },
            // { key: 'txt', label: 'TXT', icon: '📝', desc: '纯文本格式' },
            { key: 'fountain', label: 'Fountain (.fountain)', icon: '🖋️', desc: i18n.t('FOUNTAIN_FORMAT_DESC') }
        ];
        ...
        // Default to Markdown if no format is selected yet
        if (!this.selectedFormat) {
            const defaultFormatKey = 'markdown';
            const defaultButtonIndex = formats.findIndex(f => f.key === defaultFormatKey);
            if (defaultButtonIndex >= 0) {
                const defaultBtn = this.formatButtons[defaultButtonIndex];
                defaultBtn.classList.add('selected');
                this.selectedFormat = defaultFormatKey;
                this.exportSettings.format = defaultFormatKey;
                this.updateExportButtonState();
            }
        }
    }
    ...
    private updateSettingsArea() {
        ...
        if (!this.selectedFormat) {
            ...
            placeholder.innerHTML = `
                ...
                <div class="export-placeholder-text">${i18n.t('SELECT_EXPORT_FORMAT_FIRST')}</div>
            `;
            return;
        }
        ...
        if (this.selectedFormat === 'markdown' || this.selectedFormat === 'fountain') {
            this.createCleanupSettings(settingsGrid);
            return;
        }
        ...
        } else if (this.selectedFormat === 'fountain') {
            // No additional settings needed for Fountain
        }
    }

    private createCleanupSettings(container: HTMLElement) {
        const cleanupCard = container.createDiv({ cls: 'export-setting-card' });

        const cleanupHeader = cleanupCard.createDiv({ cls: 'export-setting-header' });
        cleanupHeader.innerHTML = `
            <span class="export-setting-icon" style="font-size: 18px;">🧹</span>
            <span class="export-setting-title" style="font-size: 15px;">${i18n.t('EXPORT_SETTINGS')}</span>
        `;

        const toggles = cleanupCard.createDiv({ cls: 'export-cleanup-toggles' });
        ...
        const createToggle = (
            ...
            const row = toggles.createDiv({ cls: 'export-setting-toggle' });
            const checkbox = row.createEl('input', { type: 'checkbox', attr: { id } });
        ...
        createToggle(
            'remove-frontmatter',
            i18n.t('REMOVE_FRONTMATTER'),
            this.removeFrontmatter,
            (value) => {
                this.removeFrontmatter = value;
            }
        );
    }
    ...
    private createCommonSettings(container: HTMLElement) {
        if (this.selectedFormat === 'pdf') {
            ...
            sizeHeader.innerHTML = `
                <span class="export-setting-icon" style="font-size: 18px;">📏</span>
                <span class="export-setting-title" style="font-size: 15px;">Page size</span>
            `;
    ...
    // 导出为 Fountain (.fountain)
    private async exportFountain(book: Book, rootPath: string) {
        try {
            const nodes = this.bookRenderService.collectFileNodes(book.structure.tree);
            let output = '';

            const options: ExportCleanupOptions = {
                removeComments: this.removeComments,
                removeLinks: this.removeLinks,
                removeFrontmatter: this.removeFrontmatter
            };
            ...
            for (const node of nodes) {
                ...
                if (file) {
                    ...
                    content = this.processExportContent(content, options);
                    output += content.trim() + '\n\n';
                }
            }
            ...
            link.download = `${book.basic.title}.fountain`;
            link.click();
            
            new Notice('Export ready, select save location');
            this.close();
        } catch (error) {
            console.error('Fountain export failed:', error);
            new Notice('Fountain 导出失败: ' + (error as Error).message);
        }
    }
    ...
    private async exportToDocx() {
        // DOCX export logic
        new Notice('DOCX export coming soon...');
    }

    // 导出为 Markdown
    private async exportMarkdown(book: Book, rootPath: string, options: ExportCleanupOptions) {
        const nodes = this.bookRenderService.collectFileNodes(book.structure.tree);
        ...
        const link = document.createElement("a");
        link.href = URL.createObjectURL(blob);
        link.download = `${book.basic.title}.md`;
    ...
    private processExportContent(content: string, options: ExportCleanupOptions): string {
        ...
        if (options.removeComments) {
            ...
            // Blockquotes as annotations (remove lines starting with '>')
            processed = processed
                .split('\n')
    ...
    private async handleExport() {
        if (!this.selectedFormat) {
            new Notice(i18n.t('SELECT_EXPORT_FORMAT_FIRST'));
            return;
        }

        switch (this.selectedFormat) {
            case 'markdown':
                await this.exportMarkdown(this.selectedBook, this.plugin.settings.defaultBookPath, {
                    removeComments: this.removeComments,
                    removeLinks: this.removeLinks,
                    removeFrontmatter: this.removeFrontmatter
                });
                break;
            case 'fountain':
                await this.exportFountain(this.selectedBook, this.plugin.settings.defaultBookPath);
                break;
            case 'docx':
