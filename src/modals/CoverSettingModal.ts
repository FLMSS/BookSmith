...
export class CoverSettingModal extends Modal {
    ...
    onOpen() {
        ...
        container.createEl('h3', { text: i18n.t('COVER_DESIGN') || 'Cover Design' });
        ...
        applyButton.addEventListener('click', () => {
            ...
            // 添加提示
            new Notice('Cover settings updated. Click "Re-render" to see changes.');
    ...
    private createControls(container: HTMLElement) {
        ...
        new Setting(container)
            .setName(i18n.t('COVER_IMAGE') || 'Cover Image')
            .addButton(button => button
                .setButtonText(i18n.t('SELECT_IMAGE') || 'Select Image')
                .onClick(() => this.handleImageUpload()))
            .addButton(button => button
                .setButtonText(i18n.t('CLEAR_IMAGE') || 'Clear Image')
                .onClick(() => this.handleClearImage()));
        if (this.imageUrl) {
            // 缩放控制和重置按钮（同一行）
            new Setting(container)
                .setName(i18n.t('SCALE') || 'Scale')
                .addButton(button => button
                    .setIcon('rotate-ccw') // reset icon
                    .setTooltip('Reset position')
                    .onClick(() => {
            ...
            const positionDesc = container.createEl('div', {
                ...
                text: 'Drag image to adjust position, or click reset to center'
            });
