...
export class ChapterTree {
    ...
    render(book: Book) {
        ...
        this.container.addEventListener('contextmenu', (e) => {
            // 仅在树区域空白处显示菜单（不是文件/文件夹节点）
            if ((e.target as HTMLElement).closest('.book-smith-tree-header')) {
                return;
            }

            if ((e.target as HTMLElement).closest('.book-smith-tree-list') || e.target === this.container) {
                const menu = new Menu();
                ...
                menu.addItem((item) => {
                    item.setTitle(i18n.t('OPEN_ALL_FILES_IN_ORDER'));
                    item.setIcon('list-tree');
                    item.onClick(async () => {
                        await this.openNodesInOrder(this.book.structure.tree);
                    });
                });
    ...
    private setupNodeTitle(header: HTMLElement, node: ChapterNode) {
        ...
        if (node.type === 'file') {
            ...
            const link = header.createEl('a', {
                text: node.title,
                cls: 'book-smith-tree-title book-smith-file-link internal-link',
                attr: {
                    href: filePath,
                    'data-href': filePath
                }
            ...
            link.addEventListener('click', (evt: MouseEvent) => {
                ...
                if (file instanceof TFile) {
                    const newLeaf = Keymap.isModEvent(evt); // Ctrl/Cmd = new tab
            ...
            const triggerHoverPreview = (evt: MouseEvent) => {
                ...
                if (!isMod) {
                    this.hoverTriggeredLinkPath = null;
                    return;
                }
                if (this.hoverTriggeredLinkPath === filePath) return;

                const sourcePath = this.app.workspace.getActiveFile()?.path || '';
                (this.app.workspace as any).trigger('hover-link', {
                    event: evt,
                    source: 'book-smith-chapter-tree',
                    hoverParent: this,
                    targetEl: link,
                    linktext: filePath,
                    sourcePath
                });
                this.hoverTriggeredLinkPath = filePath;
            };
    ...
    private setupContextMenu(header: HTMLElement, node: ChapterNode) {
        header.addEventListener('contextmenu', (e) => {
            ...
            if (file) {
                ...
                // Folder-only action requested above "Create copy"
                if (node.type === 'group') {
                    menu.addItem((item) => {
                        item.setTitle(i18n.t('OPEN_FOLDER_FILES_IN_ORDER'));
                        item.setIcon('list-tree');
                        item.onClick(async () => {
                            await this.openNodesInOrder(node.children || []);
                        });
                    });
                }
    ...
    private async createNode(
        ...
        try {
            if (isFile) {
                ...
                let content = '';
                if (this.book.basic.template === 'screenplay') {
                    content += '---\n';
                    content += 'cssclasses:\n';
                    content += '  - fountain\n';
                    content += '---\n\n';
    ...
    private async openNodesInOrder(nodes: ChapterNode[]) {
        const orderedFiles: TFile[] = [];

        const collect = (nodeList: ChapterNode[]) => {
        ...
        if (orderedFiles.length === 0) {
            new Notice(i18n.t('NO_FILES_TO_OPEN'));
            return;
        }

        for (let i = 0; i < orderedFiles.length; i++) {
            const leaf = this.app.workspace.getLeaf(i === 0 ? false : 'tab');
            await leaf.openFile(orderedFiles[i]);
        }
    }
}